...
function validAdminContent(){
  let name = admin_content.files[0].name;
  let type = admin_content.files[0].type;
  let size = admin_content.files[0].size;
...
function validAdminEditContent(id){

  console.log(id);

  let name = document.getElementById('admin_edit_content'+id).files[0].name;
  let type = document.getElementById('admin_edit_content'+id).files[0].type;
  let size = document.getElementById('admin_edit_content'+id).files[0].size;
  ...
  if( split_files_name.length == 2 ){
    ...
    if( 
      ...
      && size <= 128000000 ){
        ...
        document.getElementById('admin_edit_content'+id).value = '';
        return alert('Respect files format : "(A-Za-z0-9space)(.)(webm/mp4/ogv)"');
